 ...
 * This class handles the creation of a project from Excel data
 */
class CreateProjectUseCase
{
     ...
     * @throws \Exception
     */
    public function execute(array $data, array $options = []): Project
    {
        try {
            // Create project entity from Excel data
            $project = Project::createFromExcel($data);
            
            // Set post status (default to pending)
            $postStatus = $options['postStatus'] ?? PostStatus::PENDING;
            $project->updatePostStatus($postStatus);
            ...
            if (!empty($data['imagen'])) {
                $this->handleFeaturedImage($savedProject, $data['imagen']);
            }
            
            // Handle taxonomy terms
            $this->handleTaxonomies($savedProject);
            
            // Log successful creation
            debugLog(sprintf(
                'Project created: ID=%d, Title="%s", ExternalID=%s',
                $savedProject->getId(),
            ...
            debugLog('Failed to create project: ' . $e->getMessage(), 'error');
            throw $e;
        }
    }
     ...
     * @return void
     */
    private function handleFeaturedImage(Project $project, string $imageUrl): void
    {
        try {
            ...
            if ($attachmentId && !is_wp_error($attachmentId)) {
                set_post_thumbnail($project->getId(), $attachmentId);
                debugLog("Featured image set for project {$project->getId()}", 'info');
            }
     ...
     * @return void
     */
    private function handleTaxonomies(Project $project): void
    {
        // Handle research line (LGAC)
        if (!empty($project->getResearchLine())) {
            $this->assignTaxonomyTerm(
                $project->getId(),
                $project->getResearchLine(),
                RIILSA_TAXONOMY_AREA
            );
        }
        
        // Handle project status
        $statusTerm = $project->getProjectStatus()->toTaxonomyTerm();
        $this->assignTaxonomyTerm(
            $project->getId(),
            $statusTerm,
