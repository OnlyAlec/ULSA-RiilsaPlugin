...
class CreateNewsUseCase
{
    ...
    private function downloadAndAttachImage(string $imageUrl, int $postId, string $postTitle): int|false
    {
        ...
        // Special handling for Google Drive URLs which lack extension
        if (strpos($imageUrl, 'drive.google.com') !== false) {
            return $this->sideloadGoogleDriveImage($imageUrl, $postId, $postTitle);
        }
    ...
    /**
     * Manual sideload for Google Drive images
     * 
     * @param string $imageUrl
     * @param int $postId
     * @param string $postTitle
     * @return int|false
     */
    private function sideloadGoogleDriveImage(string $imageUrl, int $postId, string $postTitle): int|false
    {
        // Download to temp file
        $tempFile = download_url($imageUrl);

        if (is_wp_error($tempFile)) {
            debugLog('Google Drive download error: ' . $tempFile->get_error_message(), 'error');
            return false;
        }
        ...
        if (!$extension) {
            @unlink($tempFile);
            debugLog('Invalid image type from Google Drive: ' . $mimeType, 'error');
            return false;
        }

        $newTempFile = $tempFile;
        if (!str_contains($tempFile, $extension)) {
            $newTempFile = "$tempFile.$extension";
            rename($tempFile, $newTempFile);
        }

        $fileArgs = [
            'name' => sanitize_title($postTitle) . '.' . $extension,
            'tmp_name' => $newTempFile,
        ];

        $attachmentId = media_handle_sideload($fileArgs, $postId);

        if (is_wp_error($attachmentId)) {
            @unlink($newTempFile);
            debugLog('Google Drive sideload error: ' . $attachmentId->get_error_message(), 'error');
            return false;
        }
